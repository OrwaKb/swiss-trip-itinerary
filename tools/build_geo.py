...
STOPS = [
    # --- days 0, 1 and 8, the three airport nights and the rest day --------
    dict(slug="zrh-terminal", name="Zurich Airport, Arrivals", kind="airport",
         lat=47.45045, lon=8.56198, days=[0, 1, 2, 9],
         what="Land here Friday 20:30. Back Saturday 07:00 for the fifth traveller, Sunday for the van, and once more on the 13th.",
         osm=('nwr[aeroway=terminal]', 900)),
    dict(slug="kloten-allegra", name="Allegra Lodge, Kloten", kind="hotel",
         lat=47.45100, lon=8.58400, days=[0, 1, 8],
         what="All three airport nights: a two-bedroom suite for the 4th and 5th, a family room for the 12th. Self check-in, so a 20:30 landing needs nobody at a desk.",
         osm=('nwr[tourism=hotel][name~"Allegra",i]', 1800)),

    # --- day 6, down to Weggis and the evening boat into Lucerne -----------
    dict(slug="zrh-rental", name="Rental Centre, Airport Center Level 1", kind="parking",
         lat=47.45010, lon=8.56420, days=[2],
         what="Collect the 7-seat van here on Sunday morning, from 06:00. The first two days need no car at all.",
         osm=('nwr[amenity=car_rental]', 900)),
    dict(slug="weggis-village", name="Weggis", kind="town",
         lat=47.03330, lon=8.43330, days=[6, 7, 8],
         what="Base for the last two nights before the airport, on the north shore of Lake Lucerne.",
         osm=('nwr[place~"village|town"][name="Weggis"]', 2500)),
    dict(slug="weggis-pier", name="Weggis boat pier", kind="pier",
         lat=47.03260, lon=8.43060, days=[6],
    ...
    dict(slug="lucerne-pier", name="Lucerne, Bahnhofquai pier", kind="pier",
         lat=47.05030, lon=8.31010, days=[6],
    ...
    dict(slug="lucerne-kapellbrucke", name="Kapellbrücke, Lucerne", kind="sight",
         lat=47.05170, lon=8.30780, days=[6],
    ...
    dict(slug="weggis-cablecar", name="Weggis cable-car station", kind="lift",
         lat=47.03480, lon=8.42950, days=[7],
         what="Luftseilbahn Weggis-Rigi Kaltbad. First car up around 08:10, then every half hour - check the board, a weekday differs from a Sunday.",
         osm=('nwr[aerialway=station][name~"Weggis",i]', 2000)),
    dict(slug="rigi-kaltbad", name="Rigi Kaltbad", kind="station",
         lat=47.04660, lon=8.46920, days=[7],
    ...
    dict(slug="rigi-kanzeli", name="Kanzeli viewpoint", kind="sight",
         lat=47.04520, lon=8.46650, days=[7],
    ...
    dict(slug="meiringen", name="Meiringen", kind="town",
         lat=46.72690, lon=8.18360, days=[2, 6],
         what="Passed through on the way out, and the gorge stop on the way home.",
         osm=('nwr[place~"village|town"][name="Meiringen"]', 2500)),
    dict(slug="grindelwald-village", name="Grindelwald", kind="town",
         lat=46.62440, lon=8.04140, days=[2, 3, 4, 5, 6],
         what="Base for nights 2 to 5. Still book it first, but these dates fall before the Jodelfest rather than inside it.",
         osm=('nwr[place~"village|town"][name="Grindelwald"]', 2500)),

    # --- day 3, Männlichen and the Panoramaweg ----------------------------
    dict(slug="grindelwald-terminal", name="Grindelwald Terminal", kind="lift",
         lat=46.61110, lon=8.05530, days=[3, 4],
    ...
    dict(slug="mannlichen-top", name="Männlichen gondola station", kind="lift",
         lat=46.61360, lon=7.94110, days=[3],
    ...
    dict(slug="mannlichen-summit", name="Männlichen summit", kind="summit",
         lat=46.61560, lon=7.93640, days=[3],
    ...
    dict(slug="kleine-scheidegg", name="Kleine Scheidegg", kind="station",
         lat=46.58530, lon=7.96140, days=[3, 4],
    ...
    dict(slug="grindelwald-grund", name="Grindelwald Grund", kind="station",
         lat=46.61160, lon=8.05430, days=[3],
    ...
    # --- day 6, the gorge on the way down ---------------------------------
    dict(slug="aareschlucht-west", name="Aareschlucht West", kind="gorge",
         lat=46.72610, lon=8.19260, days=[6],
    ...
    dict(slug="aareschlucht-ost", name="Aareschlucht Ost", kind="gorge",
         lat=46.72920, lon=8.21140, days=[6],
    ...
    # --- day 8, the van goes back on the way in ---------------------------
    dict(slug="zrh-parking3", name="Zurich Airport, Parking 3", kind="parking",
         lat=47.44630, lon=8.55780, days=[8],
         what="Rental returns, accepted around the clock. Dropped Saturday on the way in, which keeps the hire to six days.",
         osm=('nwr[amenity=parking]', 700)),

    # --- days 1 and 8, the two Zurich days --------------------------------
    dict(slug="zurich-hb", name="Zürich Hauptbahnhof", kind="station",
         lat=47.37790, lon=8.54030, days=[1, 8],
         what="Thirteen minutes from the airport. Both Zurich days start here.",
    ...
    dict(slug="rheinfall", name="Rhine Falls, Schloss Laufen", kind="sight",
         lat=47.67750, lon=8.61480, days=[1],
         what="Fifty minutes direct, hourly. Optional on the rest day: lifts and a walkway drop you to a platform in the spray.",
         osm=('nwr[name~"Rheinfall",i]', 2500)),
    dict(slug="zurich-altstadt", name="Zurich Old Town, Lindenhof", kind="town",
         lat=47.37300, lon=8.54100, days=[1],
...
LEGS = [
    # Day 0 draws nothing on purpose: the van is not collected until Sunday, so
    # getting to the hotel is the shuttle, and the itinerary says 0 km.

    # --- day 1, the rest day: train in, then the old town on foot ----------
    dict(day=1, mode="lift", a="zrh-terminal", b="zurich-hb",
         label="Train to the main station, 13 minutes"),
    dict(day=1, mode="walk", a="zurich-hb", b="zurich-altstadt",
         label="Over the Limmat into the old town"),
    dict(day=1, mode="lift", a="zurich-hb", b="rheinfall",
         label="Only if everyone is up to it: direct train to the Rhine Falls, 50 minutes"),

    # --- day 2, collect the van and cross to Grindelwald -------------------
    dict(day=2, mode="walk", a="zrh-terminal", b="zrh-rental",
         label="Through to the Rental Centre on Level 1"),
    dict(day=2, mode="drive", a="zrh-rental", b="grindelwald-village",
         via=["lungern-lakeside", "meiringen"],
         label="A4 and A14 past Lucerne, A8 over the Brünig, the old lakeside road at Lungern"),

    # --- day 3, Männlichen and the Panorama Trail -------------------------
    dict(day=3, mode="walk", a="grindelwald-village", b="grindelwald-terminal",
         label="Walk or bus 121, free on the guest card"),
    dict(day=3, mode="lift", a="grindelwald-terminal", b="mannlichen-top",
         label="Männlichen gondola, 19-25 min",
         osm='way[aerialway]'),
    dict(day=3, mode="walk", a="mannlichen-top", b="mannlichen-summit",
         label="Royal Walk, 20 min up a made path"),
    dict(day=3, mode="walk", a="mannlichen-top", b="kleine-scheidegg",
         label="Panoramaweg, 4.9 km, 1h45, the whole Eiger wall on your left"),
    dict(day=3, mode="lift", a="kleine-scheidegg", b="grindelwald-grund",
    ...
    # --- day 5, Schilthorn --------------------------------------------------
    dict(day=5, mode="drive", a="grindelwald-village", b="stechelberg-parking",
         label="22 km down the valley and up the other one, about 30 min"),
    dict(day=5, mode="lift", a="stechelberg-parking", b="gimmelwald",
         label="Cable car, first section",
         osm='way[aerialway]'),
    dict(day=5, mode="lift", a="gimmelwald", b="murren-schilthornbahn",
    ...
    dict(day=5, mode="lift", a="birg", b="schilthorn",
         ...
         osm='way[aerialway]'),
    dict(day=5, mode="walk", a="murren-schilthornbahn", b="murren-blm",
         label="Through car-free Mürren, end to end"),
    dict(day=5, mode="lift", a="murren-blm", b="grutschalp",
         label="Cliff railway to Grütschalp",
         osm='way[railway~"narrow_gauge|rail"][!service]'),
    dict(day=5, mode="lift", a="grutschalp", b="lauterbrunnen",
    ...
    dict(day=5, mode="drive", a="stechelberg-parking", b="grindelwald-village",
         label="Back over to Grindelwald, the same 22 km in reverse"),

    # --- day 6, the gorge, over the Brünig, and Lucerne at dusk -----------
    dict(day=6, mode="drive", a="grindelwald-village", b="aareschlucht-west",
         label="Down to Meiringen, about 30 min"),
    dict(day=6, mode="walk", a="aareschlucht-west", b="aareschlucht-ost",
         label="Through the gorge, 1.4 km of walkway"),
    dict(day=6, mode="drive", a="aareschlucht-west", b="weggis-village",
         label="Back over the Brünig through Sarnen, then along the lake to Weggis"),
    dict(day=6, mode="walk", a="weggis-village", b="weggis-pier",
         label="Down to the pier"),
    dict(day=6, mode="boat", a="weggis-pier", b="lucerne-pier",
         label="SGV boat across the lake"),
    dict(day=6, mode="walk", a="lucerne-pier", b="lucerne-kapellbrucke",
         label="Into the old town"),

    # --- day 7, Rigi --------------------------------------------------------
    dict(day=7, mode="walk", a="weggis-village", b="weggis-cablecar",
         label="Through the village to the cable car"),
    dict(day=7, mode="lift", a="weggis-cablecar", b="rigi-kaltbad",
         label="Cable car, Weggis to Rigi Kaltbad",
         osm='way[aerialway]'),
    dict(day=7, mode="lift", a="rigi-kaltbad", b="rigi-kulm",
         label="Cogwheel to the summit, hourly",
         osm='way[railway~"narrow_gauge|rail"][!service]'),
    dict(day=7, mode="walk", a="rigi-kulm", b="rigi-kaltbad",
         label="Rigi Panorama Trail, 6 km, wide gravel, mostly downhill"),
    dict(day=7, mode="walk", a="rigi-kaltbad", b="rigi-kanzeli",
         label="Kanzeli, about an hour return, gentle"),

    # --- day 8, back to Zurich, then out to the festival ------------------
    dict(day=8, mode="drive", a="weggis-village", b="zrh-parking3",
         label="Along the lake to Küssnacht am Rigi, then A4 north past Zug"),
    dict(day=8, mode="drive", a="zrh-parking3", b="kloten-allegra",
